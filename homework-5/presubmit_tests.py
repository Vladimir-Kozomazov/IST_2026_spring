...
def test_log_reg_oracle_calls():
    ...
    (matvec_Ax, matvec_ATx, matmat_ATsA, counters) = get_counters(A)
    oracles.LogRegL2Oracle(
        matvec_Ax, matvec_ATx, matmat_ATsA, b, reg_coef
    ).func_directional(x, d, 1)
    check_counters(counters, {"Ax": 1, "ATx": 0, "ATsA": 0})
...
def test_log_reg_optimized_oracle_calls():
    ...
    # Single func
    (matvec_Ax, matvec_ATx, matmat_ATsA, counters) = get_counters(A)
    oracles.LogRegL2OptimizedOracle(
        matvec_Ax, matvec_ATx, matmat_ATsA, b, reg_coef
    ).func(x)
    check_counters(counters, {"Ax": 1, "ATx": 0, "ATsA": 0})
    ...
    # In a row: func + grad + hess
    (matvec_Ax, matvec_ATx, matmat_ATsA, counters) = get_counters(A)
    oracle = oracles.LogRegL2OptimizedOracle(
        matvec_Ax, matvec_ATx, matmat_ATsA, b, reg_coef
    )
    oracle.func(x)
    oracle.grad(x)
...
def test_hess_finite_diff_2():
    ...
    ok_(isinstance(H, np.ndarray))
    ok_(np.allclose(H, [[12.0, 0.0], [0.0, 2.0]], atol=1e-3))
...
def test_gd_1d():
    ...
    [x_star, msg, history] = optimization.gradient_descent(
        ...
        tolerance=1e-10,
        trace=True,
        line_search_options={"method": "Armijo", "alpha_0": 100, "c1": 0.3},
    )
